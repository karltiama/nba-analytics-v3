  ...
  return (
    <div className="space-y-6">
      <header className="space-y-2">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-cc-secondary">{eyebrow}</p>
        ...
        <p className="type-body max-w-3xl text-cc-secondary">{parlay.summarySentence}</p>
      </header>
      ...
      <section aria-labelledby="xray-parlay-fail-heading" className="rounded-2xl border border-[#DCE9EA] bg-white p-5" data-coachmark="why-fail">
        <h3 id="xray-parlay-fail-heading" className="type-section-heading text-[#063f46]">
        ...
        {parlay.whyThisParlayCouldFail.length === 0 ? (
          <p className="type-secondary mt-3">No cross-leg failure notes from certified packet fields.</p>
        ) : (
      ...
      <section aria-labelledby="xray-shared-context-heading" className="rounded-2xl border border-[#DCE9EA] bg-white p-5">
        <h3 id="xray-shared-context-heading" className="type-section-heading text-[#063f46]">
          Shared context
        </h3>
        <p className="type-secondary mt-1">
        ...
        {parlay.dependencyGroups.length === 0 ? (
          <p className="type-secondary mt-3">No shared player, game, or team groups on this slip.</p>
        ) : (
          <ul className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
            {parlay.dependencyGroups.map((group) => (
              <li key={group.kind + group.key} className="rounded-xl border border-[#DCE9EA] bg-[#f7f9f7] p-3 min-w-0">
                <p className="type-metadata">{parlayDependencyKindLabel(group.kind)}</p>
                <p className="type-card-data mt-1 text-[#063f46] wrap-break-word">{group.label}</p>
      ...
      {parlay.supportingContext.length > 0 ? (
        <section aria-labelledby="xray-parlay-support-heading">
          <h3 id="xray-parlay-support-heading" className="type-section-heading text-[#063f46]">
      ...
      {parlay.reviewNeeded.length > 0 ? (
        <section aria-labelledby="xray-review-heading">
          <h3 id="xray-review-heading" className="type-section-heading text-[#063f46]">
            Legs needing review
          </h3>
          <p className="type-secondary mt-1">Explicit packet flags. Not a strongest/weakest ranking.</p>
          <ul className="mt-2 space-y-2">
            {parlay.reviewNeeded.map((row) => {
              ...
              return (
                <li key={row.legIndex} className="rounded-lg border border-[#DCE9EA] bg-white px-3 py-2">
                  <a className="type-interactive text-[#075B5C] underline" href={`#xray-leg-${row.legIndex}`}>
                  ...
                  <p className="mt-1 flex flex-wrap gap-1">
                    {row.flags.map((flag) => (
                      <span
                        key={flag}
                        className="type-badge inline-flex items-center rounded-full border border-[#DCE9EA] bg-[#f7f9f7] px-2 py-0.5"
                      >
        ...
        <nav aria-label="Legs on this slip" className="flex flex-wrap gap-2">
          {interpretations.map((leg, index) => (
            <a
              key={index}
              href={`#xray-leg-${index}`}
              className="type-interactive inline-flex max-w-full items-center rounded-full border border-[#DCE9EA] bg-white px-3 py-1 text-[#063f46] hover:border-[#55ddb1] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#55ddb1]/70"
            >
      ...
      <section aria-labelledby="xray-data-coverage-heading" className="rounded-2xl border border-[#DCE9EA] bg-white p-5">
        <h3 id="xray-data-coverage-heading" className="type-section-heading text-[#063f46]">
          Data coverage
        </h3>
...
function CoverageRow({ label, have, of, hint }: { label: string; have: number; of: number; hint?: string }) {
  return (
    <li className="rounded-xl border border-[#DCE9EA] bg-[#f7f9f7] px-3 py-2">
      ...
      <p className="type-card-data mt-0.5 whitespace-nowrap text-[#063f46]">
        {have}/{of}
      </p>
      {hint ? <p className="type-metadata">{hint}</p> : null}
    </li>
  );
}
