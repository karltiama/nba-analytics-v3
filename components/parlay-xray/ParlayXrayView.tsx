...
export function ParlayXrayView({
  ...
  return (
    <main className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 overflow-x-hidden">
      {publicExtractionOff ? (
        <p
          className="type-body rounded-xl border border-[#cfeee3] bg-[#f3fbf7] px-4 py-3 text-[#063f46]"
          role="status"
        >
        ...
        <p
          className="type-body rounded-xl border border-amber-200 bg-[#fff8ee] px-4 py-3 text-[#9a3412]"
          role="status"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
        <header className="lg:col-span-3 space-y-4">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-cc-secondary">Parlay XRay</p>
          ...
          <p className="type-secondary" data-coachmark="xray-flow">
        ...
        <section className="lg:col-span-5 bg-white border border-[#DCE9EA] rounded-2xl shadow-sm p-5">
          ...
          <p className="type-body mt-1 mb-4 text-cc-secondary">
            {publicExtractionOff
          ...
          {publicExtractionOff ? (
            <p className="type-body rounded-xl border border-[#DCE9EA] bg-[#f7f9f7] px-4 py-4 text-[#063f46]">
          ...
          {hasFile && !state.designPreview && !historicalReplay && !publicExtractionOff ? (
            <div className="mt-4 space-y-2">
              <button
                ...
                onClick={onExtract}
                disabled={extracting}
                className="type-interactive inline-flex items-center justify-center rounded-xl bg-[#063f46] px-4 py-2.5 text-white shadow-sm hover:bg-[#075B5C] disabled:cursor-not-allowed disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#55ddb1]/70"
              >
          ...
          {quotaLabel && !publicExtractionOff ? <p className="type-metadata mt-3">{quotaLabel}</p> : null}
          {state.extractNotice ? (
            <p className="type-body mt-3 text-[#9a3412]" role="status">
              {state.extractNotice}
            </p>
          ) : null}
      ...
      {state.replayError ? (
        <p className="type-body rounded-xl border border-amber-200 bg-[#fff8ee] px-4 py-3 text-[#9a3412]" role="alert">
          {state.replayError}
        </p>
      ) : null}
      ...
      {hasInterpretations && state.interpretations ? (
        ...
        <section className="rounded-2xl border border-[#DCE9EA] bg-white p-5" role="status">
          <h2 className="type-section-heading text-[#063f46]">Resolving confirmed legs</h2>
          <p className="type-body mt-2 text-cc-secondary">{analysisCopy}</p>
          {state.replayError ? <p className="type-body mt-2 text-[#9a3412]">{state.replayError}</p> : null}
        </section>
      ) : historicalReplay ? (
        <section className="rounded-2xl border border-[#DCE9EA] bg-white p-5" role="status">
          <h2 className="type-section-heading text-[#063f46]">Waiting for confirmation</h2>
        ...
        <section className="rounded-2xl border border-[#DCE9EA] bg-white p-5">
          <h2 className="type-section-heading text-[#063f46]">What XRay does</h2>
...
function StageList({
  ...
  return (
    <ol className="type-secondary mt-5 space-y-2">
      <li>
