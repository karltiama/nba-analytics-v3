/**
 * Schema capability checks for collection/shadow writes.
 * Catching a PostgreSQL error does not recover an aborted transaction.
 * Inspect first; use SAVEPOINT for optional writes that may still fail.
 */
...
export type ShadowWriteSchema = {
  predictionSnapshots: boolean;
  predictionSettlements: boolean;
  shadowRunRecords: boolean;
  shadowWindowAnchors: boolean;
  ready: boolean;
  missing: string[];
};

export async function inspectShadowWriteSchema(client: SqlQueryable): Promise<ShadowWriteSchema> {
  const predictionSnapshots = await relationExists(client, 'analytics.prediction_snapshots');
  const predictionSettlements = await relationExists(client, 'analytics.prediction_settlements');
  const shadowRunRecords = await relationExists(client, 'analytics.shadow_run_records');
  const shadowWindowAnchors = await relationExists(client, 'analytics.shadow_window_anchors');
  const missing: string[] = [];
  if (!predictionSnapshots) missing.push('analytics.prediction_snapshots');
  if (!predictionSettlements) missing.push('analytics.prediction_settlements');
  if (!shadowRunRecords) missing.push('analytics.shadow_run_records');
  if (!shadowWindowAnchors) missing.push('analytics.shadow_window_anchors');
  return {
    predictionSnapshots,
...
/**
 * Run fn inside a SAVEPOINT. ROLLBACK TO SAVEPOINT on error so the outer
 * transaction remains usable. Returns null when the optional write is skipped.
 */
