...
async function fixPORGames() {
  console.log('\n🔧 Fixing Portland Past Games Status\n');
  
  const por = await pool.query(`SELECT team_id FROM teams WHERE abbreviation = 'POR'`);
  const teamId = por.rows[0].team_id;
  ...
  if (pastScheduled.rows.length > 0) {
    for (const game of pastScheduled.rows) {
      const vs = game.home_team_abbr === 'POR' ? 'vs' : '@';
      const opponent = game.home_team_abbr === 'POR' ? game.away_team_abbr : game.home_team_abbr;
