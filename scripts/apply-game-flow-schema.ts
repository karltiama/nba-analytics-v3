/**
 * Apply analytics.game_flow empty serving schema (Step 12G).
...
const sql = fs.readFileSync(path.join('db', 'schemas', 'MIGRATION_game_flow.sql'), 'utf8');
...
async function main() {
  ...
  try {
    ...
    const table = await pool.query<{ rel: string | null }>(
      `select to_regclass('analytics.game_flow')::text as rel`
    );
    const indexes = await pool.query<{ indexname: string }>(
      `select indexname from pg_indexes
       where schemaname = 'analytics' and tablename = 'game_flow'
       order by indexname`
    );
    const rls = await pool.query<{ relrowsecurity: boolean }>(
      `select c.relrowsecurity
       ...
       where n.nspname = 'analytics' and c.relname = 'game_flow'`
    );
    const grants = await pool.query<{ grantee: string; privilege_type: string }>(
      `select grantee, privilege_type
       from information_schema.role_table_grants
       where table_schema = 'analytics' and table_name = 'game_flow'
       order by grantee, privilege_type`
    );
    console.log('MIGRATION_OK');
    console.log(`game_flow=${table.rows[0]?.rel}`);
