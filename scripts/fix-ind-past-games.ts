...
async function fixINDGames() {
  console.log('\n🔧 Fixing Indiana Past Games Status\n');
  
  const ind = await pool.query(`SELECT team_id FROM teams WHERE abbreviation = 'IND'`);
  const teamId = ind.rows[0].team_id;
  ...
  if (pastScheduled.rows.length > 0) {
    for (const game of pastScheduled.rows) {
      const vs = game.home_team_abbr === 'IND' ? 'vs' : '@';
      const opponent = game.home_team_abbr === 'IND' ? game.away_team_abbr : game.home_team_abbr;
