...
async function fixDenverGames() {
  console.log('\n🔧 Fixing Denver Past Games Status\n');
  
  const denver = await pool.query(`SELECT team_id FROM teams WHERE abbreviation = 'DEN'`);
  const teamId = denver.rows[0].team_id;
  ...
  for (const game of pastScheduled.rows) {
    const vs = game.home_team_abbr === 'DEN' ? 'vs' : '@';
    const opponent = game.home_team_abbr === 'DEN' ? game.away_team_abbr : game.home_team_abbr;
  ...
  const needStats = pastScheduled.rows.filter((g: any) => !g.has_stats);
  ...
  console.log(`\n📊 Denver Summary:`);
  console.log(`   Completed games: ${completed.rows[0].count}`);
  console.log(`   Games with stats: ${withStats.rows[0].count}`);
  console.log(`   Coverage: ${completed.rows[0].count > 0 ? Math.round((parseInt(withStats.rows[0].count) / parseInt(completed.rows[0].count)) * 100) : 0}%`);
