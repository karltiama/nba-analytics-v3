...
async function fixNOPGames() {
  console.log('\n🔧 Fixing New Orleans Past Games Status\n');
  
  const nop = await pool.query(`SELECT team_id FROM teams WHERE abbreviation = 'NOP'`);
  const teamId = nop.rows[0].team_id;
  ...
  if (pastScheduled.rows.length > 0) {
    for (const game of pastScheduled.rows) {
      const vs = game.home_team_abbr === 'NOP' ? 'vs' : '@';
      const opponent = game.home_team_abbr === 'NOP' ? game.away_team_abbr : game.home_team_abbr;
