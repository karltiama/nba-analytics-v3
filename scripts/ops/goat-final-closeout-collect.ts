/**
 * Read-only Step 10 collector. Lists S3 + fetches manifests.
 * Does not import BdlArchiveClient. No Postgres writes. No BDL HTTP.
 *
 *   npx tsx scripts/ops/goat-final-closeout-collect.ts
 */
...
const OUT = 'reports/trial/goat-final-closeout-s3-snapshot.json';
...
async function main() {
  ...
  const prefixes = [
    ...
    'raw/source=balldontlie/league=nba/season=2025/entity=plays',
  ...
  const manifestKeys = [
    ...
    'raw/source=balldontlie/league=nba/season=2025/entity=plays/_manifest.json',
    'raw/source=balldontlie/league=nba/season=2025/entity=plays/_run.json',
  ...
  for (const key of manifestKeys) {
    ...
    manifests.push({
      key,
      present: body != null,
      status: body?.status ?? body?.completionStatus ?? body?.acquisitionStatus ?? null,
      ...
      recordCount:
        ...
        body?.totalRecords ??
        body?.totalEvents ??
        body?.certifiedEventsSum ??
        null,
      targetGames: body?.targetGames ?? body?.targetGameCount ?? body?.targetCount ?? null,
      season: body?.season ?? null,
      entity: body?.entity ?? null,
      availabilityLimitation: body?.availabilityLimitation ?? null,
      scopeLimitation:
        body?.scopeLimitation ??
        body?.availabilityLimitation ??
        body?.limitationAfterMarch22 ??
        body?.notAFullSeasonArchive ??
        null,
    ...
    JSON.stringify(
      {
        out: OUT,
        objectCount,
        totalMb: snapshot.bucket.totalMb,
        totalGb: snapshot.bucket.totalGb,
        lockActive: lock.active,
        frozen: snapshot.safety.frozen,
        season: snapshot.safety.currentAnalyticsSeason,
      },
      null,
      2
    )
