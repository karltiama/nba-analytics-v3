...
async function fixPHXGames() {
  console.log('\n🔧 Fixing Phoenix Past Games Status\n');
  
  const phx = await pool.query(`SELECT team_id FROM teams WHERE abbreviation = 'PHX'`);
  const teamId = phx.rows[0].team_id;
  ...
  if (pastScheduled.rows.length > 0) {
    for (const game of pastScheduled.rows) {
      const vs = game.home_team_abbr === 'PHX' ? 'vs' : '@';
      const opponent = game.home_team_abbr === 'PHX' ? game.away_team_abbr : game.home_team_abbr;
