...
async function findMissingGames() {
  ...
  // Also check games that might be missing player stats
  const missingPlayerStats = completedGames.rows.filter((g: any) => !g.has_player_stats);
  
  if (missingPlayerStats.length > 0) {
    console.log(`\n⚠️  Games missing player stats: ${missingPlayerStats.length}`);
    missingPlayerStats.forEach((game: any) => {
      const vs = game.home_team_abbr === 'UTA' ? 'vs' : '@';
      const opponent = game.home_team_abbr === 'UTA' ? game.away_team_abbr : game.home_team_abbr;
