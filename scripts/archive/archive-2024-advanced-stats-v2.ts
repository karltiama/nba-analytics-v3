/**
 * Step 5C: complete season-2024 Advanced Stats V2 → S3. No Postgres writes.
 * Resumes past Step 5A pages 1–5. Does not fetch 2023. Does not start props/odds/lineups/2022.
 *
 *   BDL_TRIAL_MODE=1 npx tsx scripts/archive/archive-2024-advanced-stats-v2.ts --dry-run
 *   BDL_TRIAL_MODE=1 npx tsx scripts/archive/archive-2024-advanced-stats-v2.ts --execute
 */
...
const SEASON = 2024;
const OUT_JSON = 'reports/trial/advanced-stats-2024-archive-report.json';
const OUT_MD = 'reports/trial/advanced-stats-2024-archive-report.md';
const SAMPLE_5A = 'reports/trial/advanced-stats-characterization.json';
const REPORT_2025 = 'reports/trial/advanced-stats-2025-archive-report.json';
const TRIAL_START = '2026-09-08T12:10:59.422Z';
const TRIAL_HOURS = 48;
const RESERVE_HOURS = 6;
const TIME_GATE_MAX_HOURS = 2;
const EXPECTED_DB_BYTES = 342846611;
const COVERAGE_2025 = 75.6;
const FORECAST = {
  pages: 350,
  records: 34881,
  practicalSpacingSec: 13.84,
  s3Mb: 173,
  hours: 1.35,
  baseline2025: { pages: 349, records: 34810, durationHours: 1.34, s3Mb: 172.07 },
};
const CORE_FIELDS = [
  'possessions',
  'usage_percentage',
...
async function main() {
  ...
  if (!plan.s3Prefix.endsWith('season=2024/entity=advanced_stats_v2')) unsafe.push('wrong S3 prefix');
  ...
  if (before.dbBytes !== EXPECTED_DB_BYTES) unsafe.push(`db bytes ${before.dbBytes} not ${EXPECTED_DB_BYTES}`);
  ...
  const pageIndexesBefore = pageKeysBefore
    .map((o) => Number((o.key.match(/page=(\d+)\.json$/) ?? [])[1] ?? 0))
    .sort((a, b) => a - b);
  const characterizationManifest = prefixObjects.find((o) =>
    o.key.endsWith('/_characterization_5page_manifest.json')
  );
  const completeManifestBefore = prefixObjects.find((o) => o.key.endsWith('/_manifest.json'));
  const preserved = prefixObjects.filter(
    (o) => !/\/page=\d+\.json$/.test(o.key) && !o.key.endsWith('/_manifest.json')
  );
  const s3BytesBefore = prefixObjects.reduce((a, o) => a + o.bytes, 0);
  const remainingPagesEst = Math.max(FORECAST.pages - pageIndexesBefore.length, 0);
  const remainingHigh = Math.ceil(remainingPagesEst * 1.15) || FORECAST.pages;
  const projectedBaseHours = hoursFor(remainingPagesEst || FORECAST.pages, FORECAST.practicalSpacingSec);
  const projectedHighHours = hoursFor(remainingHigh, FORECAST.practicalSpacingSec);
  const projectedRemainingAfter = hours(remainingHours - projectedHighHours);
  const resumeIntended =
    pageIndexesBefore.length > 0 &&
    pageIndexesBefore[0] === 1 &&
    pageIndexesBefore.every((n, i) => n === i + 1) &&
    !completeManifestBefore;
  if (pageIndexesBefore.length > 0 && pageIndexesBefore[0] !== 1) {
    unsafe.push(`existing pages do not start at 1: ${pageIndexesBefore.slice(0, 8).join(',')}`);
  }
  if (completeManifestBefore && pageIndexesBefore.length < 300) {
    unsafe.push('complete _manifest.json present with too few pages — refusing to treat characterization as complete');
  }

  const characterizationHandling = {
    existingPages: pageIndexesBefore,
    characterizationManifest: characterizationManifest?.key ?? null,
    completeManifestBefore: completeManifestBefore?.key ?? null,
    strategy: resumeIntended
      ? 'resume: keep pages 1–N as canonical, continue cursor from last page next_cursor, write complete _manifest.json at exhaustion, preserve _characterization_5page_manifest.json'
      : pageIndexesBefore.length === 0
        ? 'fresh crawl from page 1'
        : 'resume from last archived page (skip-existing HTTP), preserve non-page characterization objects',
    willNotTreatCharacterizationManifestAsComplete: true,
    samePrefix: true,
  };
  ...
  const timeGate = {
    ...
    existingPages: pageIndexesBefore.length,
    remainingPagesEst,
    forecast: FORECAST,
    projectedBaseHours,
    projectedHighHours,
    ...
    writeStopped(
      {
        ...
        step: '5C',
        stopped: true,
        reason: [`projected 2024 crawl ${projectedHighHours}h exceeds ${TIME_GATE_MAX_HOURS}h gate`],
        safety,
        timeGate,
        characterizationHandling,
        verdict: 'RED — stop Advanced Stats progression',
  ...
  if (dryRun) {
    const payload = {
      ...
      note: 'Safety and time gate GREEN. Rerun with --execute. Resumes 2024 from existing pages; crawls to cursor exhaustion.',
    };
    mkdirSync('reports/trial', { recursive: true });
    writeFileSync(OUT_JSON, JSON.stringify(payload, null, 2) + '\n');
    console.log(JSON.stringify(payload, null, 2));
  ...
  try {
    console.log(
      `[5C] archiving 2024 Advanced Stats V2 to S3 (resume from page ${pageIndexesBefore.length + 1}; no Postgres writes)`
    );
  ...
  for (const field of [...CORE_FIELDS, ...TRACKING_FIELDS]) {
  ...
  let logMatch = {
    ...
    matchRatePct: 0,
    coverageVsLogsPct: 0,
    vs2025CoveragePct: COVERAGE_2025,
    coverageClass: 'similar' as 'similar' | 'moderately different' | 'materially different',
    advancedOnlySample: [] as string[],
  };
  let gameCoverage = {
    analyticsGames2024: 0,
    advancedDistinctGames: gameIds.size,
    gamesWithAtLeastOneAdvanced: 0,
    gamesWithZeroAdvanced: 0,
    zeroAdvancedIds: [] as string[],
    advancedIdsAbsentFrom2024: [] as string[],
    wrongSeasonIds: [] as string[],
  };
  let after = before;
  try {
    ...
    const logs = await idClient.query<{ game_id: string; player_id: string; team_id: string }>(
      `select game_id, player_id, team_id from analytics.player_game_logs where season = '2024'`
    );
    ...
    identity = {
      games: {
        ...
        wrongSeason: [...gameIds].filter((id) => mappedG.has(id) && mappedG.get(id) !== '2024'),
    ...
    const analyticsGames = await idClient.query<{ game_id: string }>(
      `select game_id from analytics.games where season = '2024'`
    );
    const analyticsGameSet = new Set(analyticsGames.rows.map((r) => String(r.game_id)));
    const zeroAdvancedIds = [...analyticsGameSet].filter((id) => !gameIds.has(id)).sort();
    const advancedIdsAbsentFrom2024 = [...gameIds].filter((id) => !analyticsGameSet.has(id)).sort();
    gameCoverage = {
      analyticsGames2024: analyticsGameSet.size,
      advancedDistinctGames: gameIds.size,
      gamesWithAtLeastOneAdvanced: [...analyticsGameSet].filter((id) => gameIds.has(id)).length,
      gamesWithZeroAdvanced: zeroAdvancedIds.length,
      zeroAdvancedIds: zeroAdvancedIds.slice(0, 50),
      advancedIdsAbsentFrom2024,
      wrongSeasonIds: [...gameIds].filter((id) => mappedG.has(id) && mappedG.get(id) !== '2024'),
    };
    const coveragePct = logKey.size ? Math.round((matching / logKey.size) * 1000) / 10 : 0;
    const covDelta = Math.abs(coveragePct - COVERAGE_2025);
    const coverageClass =
      covDelta < 5 ? 'similar' : covDelta < 10 ? 'moderately different' : 'materially different';
    logMatch = {
      advancedKeys: gp.size,
      matchingLogs: matching,
      advancedOnly,
      logOnly,
      logs2024: logKey.size,
      matchRatePct: gp.size ? Math.round((matching / gp.size) * 1000) / 10 : 0,
      coverageVsLogsPct: coveragePct,
      vs2025CoveragePct: COVERAGE_2025,
      coverageClass,
      advancedOnlySample,
    };
  ...
  const runMeta = {
    ...
    params: { 'seasons[]': '2024', period: 0, per_page: 100 },
    characterizationHandling,
  ...
  const meanPages = (pageKeys.length + 349) / 2;
  const meanRecords = (rows + 34810) / 2;
  const meanS3Mb = (mb(s3Bytes) + 172.07) / 2;
  const rec2023 = {
    method: 'mean of complete 2025 and 2024 seasons, scaled by 2023 log count',
    pages: Math.ceil((meanPages * LOGS[2023]) / ((LOGS[2025] + LOGS[2024]) / 2)),
    recordsEst: Math.round((meanRecords * LOGS[2023]) / ((LOGS[2025] + LOGS[2024]) / 2)),
    practicalHours: hoursFor(
      Math.ceil((meanPages * LOGS[2023]) / ((LOGS[2025] + LOGS[2024]) / 2)),
      spacingSec
    ),
    s3Mb: Math.round(((meanS3Mb * LOGS[2023]) / ((LOGS[2025] + LOGS[2024]) / 2)) * 100) / 100,
  };
  ...
  if (gameCoverage.advancedIdsAbsentFrom2024.length) {
    redReasons.push(`advanced-only games ${gameCoverage.advancedIdsAbsentFrom2024.join(',')}`);
  }
  if (logMatch.coverageClass === 'materially different') {
    yellowReasons.push(`log coverage ${logMatch.coverageVsLogsPct}% vs 2025 ${COVERAGE_2025}% (${logMatch.coverageClass})`);
  }
  if (metrics.status429 > 0) yellowReasons.push(`${metrics.status429} 429s`);

  let verdict:
    | 'GREEN — 2024 Advanced Stats archived successfully; proceed to 2023'
    | 'YELLOW — archive complete but review material difference before continuing'
    | 'RED — stop Advanced Stats progression';
  if (redReasons.length) verdict = 'RED — stop Advanced Stats progression';
  else if (yellowReasons.length) verdict = 'YELLOW — archive complete but review material difference before continuing';
  else verdict = 'GREEN — 2024 Advanced Stats archived successfully; proceed to 2023';
  ...
  let quality2025: Array<{ field: string; pctPopulated: number }> = [];
  try {
    const r2025 = JSON.parse(readFileSync(REPORT_2025, 'utf8')) as {
      fieldQuality?: { fields?: Array<{ field: string; pctPopulated: number }> };
    };
    quality2025 = r2025.fieldQuality?.fields ?? [];
  } catch {
    quality2025 = [];
  }
  const pctOf = (
    list: Array<{ field: string; pctPopulated: number }>,
    field: string
  ): number | null => list.find((q) => q.field === field)?.pctPopulated ?? null;
  const fieldCompare = [...CORE_FIELDS, ...TRACKING_FIELDS].map((field) => ({
    field,
    sample5a: pctOf(
      (sample5a?.fieldQuality?.numeric ?? []).map((q) => ({ field: q.field, pctPopulated: q.pctPopulated })),
      field
    ),
    full2024: pctOf(quality, field),
    full2025: pctOf(quality2025, field),
  }));
  const deadFields = quality.filter((q) => q.nonNull > 0 && q.min === q.max);
  const trackingPop = TRACKING_FIELDS.map((f) => pctOf(quality, f)).filter((n): n is number => n != null);
  const corePop = CORE_FIELDS.map((f) => pctOf(quality, f)).filter((n): n is number => n != null);
  const trackingPopAvg = trackingPop.length
    ? Math.round((trackingPop.reduce((a, b) => a + b, 0) / trackingPop.length) * 10) / 10
    : null;
  const corePopAvg = corePop.length
    ? Math.round((corePop.reduce((a, b) => a + b, 0) / corePop.length) * 10) / 10
    : null;
  ...
  const payload = {
    ...
    characterizationHandling: {
      ...characterizationHandling,
      after: {
        characterizationManifestPreserved: prefixAfter.some((o) =>
          o.key.endsWith('/_characterization_5page_manifest.json')
        ),
        completeManifest: `${plan.s3Prefix}/_manifest.json`,
        pages1to5ReusedAsCanonical: pageIndexesBefore.includes(1),
      },
    },
    timeGate,
    acquisition: {
    ...
    fieldQuality: {
      fields: quality,
      coreAvgPct: corePopAvg,
      trackingAvgPct: trackingPopAvg,
      compare5aVsFull2024VsFull2025: fieldCompare,
      sampleVsFull2024: substantialChanges,
      switches_on: quality.find((q) => q.field === 'switches_on') ?? null,
      deadOrConstantWhenPopulated: deadFields
        .filter((q) => q.field !== 'period' && q.field !== 'id')
        .map((q) => ({
          field: q.field,
          value: q.min,
          pctPopulated: q.pctPopulated,
        })),
      topLevelKeyCount: topLevelKeys.size,
    },
    comparison2024vs2025: {
      pages: { 2025: 349, 2024: pageKeys.length },
      records: { 2025: 34810, 2024: rows },
      games: { 2025: 1322, 2024: gameIds.size },
      players: { 2025: 594, 2024: playerIds.size },
      logCoveragePct: { 2025: COVERAGE_2025, 2024: logMatch.coverageVsLogsPct },
      coreFieldPopulationPct: { 2025: 99.9, 2024: corePopAvg },
      trackingPopulationPct: { 2025: '77-80', 2024: trackingPopAvg },
      s3Mb: { 2025: 172.07, 2024: mb(s3Bytes) },
      timeHours: { 2025: 1.34, 2024: actualHours },
    },
    outlierProfile: {
    ...
    s3Growth: {
      before: { objects: prefixObjects.length, bytes: s3BytesBefore, mb: mb(s3BytesBefore), pages: pageIndexesBefore.length },
      after: { objects: prefixAfter.length, bytes: s3Bytes, mb: mb(s3Bytes), pages: pageKeys.length },
      incrementalBytes: s3Bytes - s3BytesBefore,
      incrementalMb: mb(s3Bytes - s3BytesBefore),
      characterizationObjectsPreserved: preserved.map((o) => o.key),
      canonicalPageObjects: pageKeys.length,
    },
    ...
    trialTimeBudget: {
      ...
      estimated2023AdvancedHours: rec2023.practicalHours,
      openingPropsHours: { low: 3.0, high: 5.3 },
      openingOddsHours: { low: 0.8, high: 5.0 },
      optionalLineupHours: 5,
      optional2022: 'S3-only; drop first if short',
      priority: [
        'finish Advanced 2024',
        'finish Advanced 2023',
        'historical opening player props',
        'historical opening odds',
  ...
  writeFileSync(
    OUT_MD,
    [
      '# 2024 Advanced Stats V2 archive (Step 5C)',
      '',
      `Generated: ${generatedAt}`,
      '',
      `**${verdict}**`,
      '',
      `- pages ${pageKeys.length}; records ${rows}; exhausted ${exhausted}; HTTP ${metrics.httpAttempts}`,
      `- coverage ${logMatch.coverageVsLogsPct}% (${logMatch.coverageClass} vs 2025 ${COVERAGE_2025}%)`,
      `- S3 ${mb(s3Bytes)} MB (delta ${mb(s3Bytes - s3BytesBefore)} MB); Postgres delta ${bytesDelta} bytes`,
      '',
      'Do not start 2023. Do not write Advanced Stats to Postgres.',
      '',
    ].join('\n')
  );
  console.log(
