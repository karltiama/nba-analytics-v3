...
export function PlayerTrendChart({
  ...
  if (data.length === 0) {
    return (
      <div
        className="bg-white border border-[#DCE9EA] rounded-2xl shadow-sm p-8 flex items-center justify-center"
        style={{ minHeight: CHART_HEIGHT + 60 }}
      >
        <span className="text-[#4a6366]">Not enough data yet</span>
      </div>
    );
  }
            ...
            <div className="hidden lg:block h-20">
              {displayIndex !== null ? (
                <div className="relative p-2 rounded-lg bg-[#F8FBFA] border border-[#DCE9EA] font-mono h-full flex flex-col justify-center items-center text-center min-w-0 gap-0.5">
                  ...
                  <div className="text-xl font-bold text-[#075B5C] leading-tight">{data[displayIndex]} {metricLabel}</div>
